...
class writer(object):
    def __init__(self, folder='./output/', filename='heat_eqn.h5'):
...
def get_args():
    ...
    parser.add_argument('-ic', '--initial_conditions', action='store', dest='ic', help='<Required> Set initial conditions', required=True,choices={'random', 'block', 'gauss'})

    args = parser.parse_args() # collect cmd line args
    ic = args.ic

    if   ic == 'random':
        # from input.random import UserData, sol_init
        raise NotImplementedError("random IC not yet implemented")
    elif ic == 'block': 
        # from input.block import UserData, sol_init
        raise NotImplementedError("block IC not yet implemented")
    elif ic == 'gauss':
        from input.gauss import UserData, sol_init
